...
public class Seed extends Node {
	...
	public void body() {
		...
		this.oppnetTasks.add("Goal Urgent Circulate fresh air"); // غيرنا المسج وكلمات المقارنة رح تكون كلمتين مستخدمين بكلاس الهيلبر
		...
		pr.oppnet.add(this); // استخدمنا الاوبجكت من كلاس البراميتر اللي عملناه فوق لحتى نكمل اضافة على ارآي ليست الاوبنت تاسك 
		this.tasksRunTimes.add(pr.task1Runtime);  // ضفناالتاسك الاولى للاراي ليست
		...
		for (int i = 0; i < pr.Helpers.size(); i++) {
			...
			if ((pr.Helpers.get(i).Comm_Channels.contains("Zigbee"))  // كانت بلوتوث 
		...
		int i = 0;
// while loop will discover each Arraylist items of candidates helper
		while (i < CandidateHelpers.size()) {

			// Step 1.3.1: seed discovers services in candidate
                                                                      //كانت بلوتوث
			if ((CandidateHelpers.get(i).Comm_Channels.contains("Zigbee") && 
                                CandidateHelpers.get(i).Coordinator)) { 
				...
				if (this.pr.oppnet.contains(Candidate_Helper)) {
					...
					// helper (Node_regHelp)
                                        ...
                                        // هون انتبه تغيرت من اكتشوال تبع بلوتوث للنوع تبعنا
                                        // change from pr.actualBTdelTimeCtrlMsg To pr.actualZigbeeDelTimeCtrlMsg
					...
					sim_trace(1,
							"Step 1.3.3 Done: " + this.get_name()
									...
									+ Candidate_Helper.get_name() + ", "
									+ Candidate_Helper.get_id());
					Candidate_Helper.linkWithParent = "Zigbee";//غيرته للزيبقي
					Candidate_Helper.ctrlMsgDelTimeThroughLinkToParent = pr.actualZigbeeDelTimeCtrlMsg;
				}
			}// end if(CandidateHelpers.get(i).Comm_Channels.contains("Zigbee") &&
				// CandidateHelpers.get(i).C)
...
graph.comm.add(Candidate_Helper.linkWithParent);
       ...
       // frame.setContentPane(new JLabel(new ImageIcon(ImageIO.read(new File("tu.gif")))));
		...
		// Step 1.13 release admitted helpers
                // Notice change in communication channel from Bluetooth To Zigbee. pr.actualZigbeeDelTimeCtrlMsg
		...
		sim_trace(1,
				"Step 1.15 Done: " + this.get_name() + ", " + this.get_id()
                        ...
                        pr.oppnet.clear();
                        sim_trace(1,"The CO2 Level is NORMAL, it's between 0.1 and 0.6 ..  "+this.get_name()
                        + ", " +this.get_id()+ " terminated oppnet");
                        }
