...
ArrayList<String> he= new ArrayList<String>();
                ...
                he.add(Candidate_Helper.linkWithParent);
				  ...
				  this.hlprIntegTime.add((endTime-startTime));
                    he.add(String.valueOf((endTime - startTime)));
                                        graph.node.add(he);
				  //Step 1.9: seed sends help object to admitted helper (Node_report)

                   this.tasksRunTimes.add(this.task2Runtime);                
                   this.NODE_report(Candidate_Helper, pr.task2RuntimeSeedZigBee, 19, helpObject);      
                  ...
                  //Step 1.10: seed sends oppnet tasks to admitted helper (Node_report)
                   this.NODE_sendData(Candidate_Helper, pr.task3RuntimeSeedZigBee, 110, oppnetTasks);      
                   ...
                   sim_trace(1, "Step 1.10 Done: "+ this.get_name()+", "+ this.get_id()+" sent oppnet tasks to "+ Candidate_Helper.get_name() + ", "+ Candidate_Helper.get_id());
                          ...
                          //break; //use a break if a single helper is integrated since it is a single helper mode
...
/*graph frame = new graph();
       ...
       // frame.setContentPane(new JLabel(new ImageIcon(ImageIO.read(new File("tu.gif")))));
        ...
        frame.setVisible(true);*/
                ...
                } //end if that tests the level of co2
                
                else{
                         //Step 1.15: Terminate oppnet       
                    pr.oppnet.clear();
                    sim_trace(1, "Step 1.15: The Co2 level is between 0.1% and 0.6% "+ this.get_name() + ", " + this.get_id()+"terminated oppnet");      
        }
