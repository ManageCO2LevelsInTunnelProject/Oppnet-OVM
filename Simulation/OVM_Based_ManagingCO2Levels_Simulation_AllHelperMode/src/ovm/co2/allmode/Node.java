...
//////////////////////////////////////////////////////////////////////////
	   ...
	   public Parameters NODE_initiate(Seed s){
			...
			s.oppnetTasks.add("Goal Urgent Circulate fresh air");
	  ...
	  public boolean NODE_runApp(Helper h, String DeviceTask, Help helpObject, Parameters pr){
		...
		//  int j = 0;
		 // while(j< DeviceTasks.size()){
		     if (DeviceTask.contains("Circulate")){
      	      sim_process(pr.task6Runtime);
      	      h.performedUrgent = true;
      	      h.tasksRunTimes.add(pr.task6Runtime);
              graph.nodeNames.add(h);
              //Done = true;
		  		
  	         } //end if (DeviceTasks.get(i).equals("ciculate")
  	   	if (DeviceTask.contains("Display") && this.device.equals("Car")||this.device.equals("Smartphone")){
		System.out.println("Displaying help message from req helper phone or car\n" + helpObject.helpTextMsg.toString());
	  	...
	  	if (DeviceTask.contains("Display")&& this.device.equals("Billboard")||this.device.equals("Billboard2")){
	        System.out.println("Displaying help message from lite helper billboard" + helpObject.helpTextMsg.toString());
      	        sim_process(pr.task5Runtime);
      	        h.performedUnhurried = true;
      	        h.tasksRunTimes.add(pr.task5Runtime);

  	        } //end if (DeviceTasks.get(i).equals("Display on billboard"))
				...
				for (int i = 0; i < DeviceApps.size(); i++) {
					...
					if (oppnetTasks.get(ot).contains(DeviceApps.get(i))) {
						...
						sim_trace(1, "Step 2.10: " + this.get_name() + ", "
								...
								+ DeviceTasks.get(j));
						if (DeviceTasks.get(j).contains("ciculate") )) {
