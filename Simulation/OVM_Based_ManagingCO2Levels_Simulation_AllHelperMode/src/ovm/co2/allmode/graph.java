        ...
        //    stylesheet.putCellStyle( styleName, style);
            ...
            for(int ii=0; ii<node.size();ii++){
               ...
               vertex.add(vertex1);}
                   ...
                   ArrayList<Object> vertex1= new ArrayList<Object>();
                   Object v= graph.insertVertex(parent, null, "Car", nodeLoc.get(ii).X*5, nodeLoc.get(ii).Y*5, 80, 50, "shape=image;image=/car.png");
                vertex1.add(v);
                vertex1.add(node.get(ii).get(1));
               vertex1.add(node.get(ii).get(2));
               vertex1.add(node.get(ii).get(3));
               ...
               if(vertex.get(ii+1).get(2).equals(vertex.get(i).get(1))){ 
                   ...
                   for(int nu=0;nu<nodeNames.size();nu++){
                       if(vertex.get(ii+1).get(1).equals(String.valueOf(nodeNames.get(nu).get_id()))){
                         ...
                         graph.insertEdge(parent, null,vertex.get(ii+1).get(3)+" \n"+vertex.get(ii+1).get(4), vertex.get(i).get(0), vertex.get(ii+1).get(0),"strokeColor=#00FF00;");   
...
graph.setStylesheet(stylesheet);*/
            ...
            graph.insertEdge(parent, null,vertex.get(ii+1).get(3)+" \n"+vertex.get(ii+1).get(4), vertex.get(i).get(0), vertex.get(ii+1).get(0),"strokeColor=#FF0000;");}}}
