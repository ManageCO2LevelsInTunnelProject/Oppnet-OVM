...
ArrayList <Double> integ = new ArrayList<Double>();
	...
	public void body() {
		...
		if (ev.get_data() != null) {
sim_trace(1,"Step 2.1 Done: " + this.get_name() + ", " + this.get_id()+ 
                " received a message from a node with id "+ ev.get_src());
...
sim_trace(1, "Step 2.2 Done: "+ this.get_name()+ ", "+ this.get_id()+
...
sim_schedule(Inviting_Node.get_id(),this.ctrlMsgDelTimeThroughLinkToParent, 
        23, "Join");
sim_trace(1,"Step 2.3 Done: " + this.get_name() + ", "+ this.get_id()+
" accepted invitation of "+ Inviting_Node.get_name() + ", "
        + Inviting_Node.get_id());
					...
					sim_get_next(ev);
					if (ev.get_data() != null) {
 sim_trace(1, "Step 2.4 Done: " + this.get_name() + ", "+ this.get_id()+
         " received a message from a node with id "+ ev.get_src());
...
sim_trace(1,"Step 2.5 Done: "+ this.get_name()+ ", "+ this.get_id()+ 
...
+ Sim_system.get_entity(ev.get_src()).get_name()+ ", "+
        Sim_system.get_entity(ev.get_src()).get_id());
			...
			Inviting_Node = (Node) Sim_system.get_entity(ev.get_src());
                        ...
                        graph.nodeNames.add(Inviting_Node);
                                        
               /* graph.loc.add(Inviting_Node.location);/*
              
               System.out.println("--------------------------------------------------------------------------------"+Inviting_Node.device);*/}}
		...
		if (admitted) {
             ...
             //  graph.nodeNames.add(this.device);
                //graph.nodeLoc.add(this.location);
...
" processes the message and it is the oppnet tasks from "
     + Sim_system.get_entity(ev.get_src()).get_name() + ","+ ev.get_src());
			...
			if (this.DeviceApps.size() > 0) {
					...
					for (int i = 0; i < DeviceApps.size(); i++) {
						...
						if (oppnetTasks.get(ot).contains(DeviceApps.get(i))) {
							...
							if (DeviceTasks.get(j).contains("Circulate")) {
								sim_process(pr.task6Runtime);
								this.performedUrgent = true;
								this.tasksRunTimes.add(pr.task6Runtime);
                                                             graph.nodeNames.add(this);
                                        
                graph.loc.add(this.location);   
                                                 

							} // end if (DeviceTasks.get(i).equals("SMS")

					          if (DeviceTasks.get(j).contains("Display")&&this.get_name()!="Billboard"&&this.get_name()!="Billboard2") {
							...
							if (DeviceTasks.get(j).contains("Display")&&(this.get_name()=="Billboard"||this.get_name()=="Billboard2")) {
				System.out.println("Displaying help message in Billboard"
										+ helpObject.helpTextMsg.toString());
				...
				sim_trace(1,"Step 2.10: " + this.get_name() + ", "+ this.get_id() +
                                        " run app "+ DeviceTasks.get(j));
			...
			if (Done) {
				sim_schedule(Inviting_Node.get_id(),this.ctrlMsgDelTimeThroughLinkToParent,
                                        900,"Done..ReqRelease");
				sim_trace(1,"Step 2.11: " + this.get_name() + ", " + this.get_id()+
                  " Sent Done notification to "+ Inviting_Node.get_name() +
                                        ", "+ Inviting_Node.get_id());
		...
		"Step 2.12: " + this.get_name() + ", " + this.get_id()+
				...
				for (int c = 0; c < this.pr.Helpers.size(); c++) {
					...
					for (int x = 0; x < this.Comm_Channels.size(); x++) {
						for (int k = 0; k < this.pr.Helpers.get(c).Comm_Channels.size(); k++) {
					if (this.Comm_Channels.get(x).equals(this.pr.Helpers.get(c).Comm_Channels.
   get(k))&& distance.X <= 100 && distance.Y <= 100&& !(pr.oppnet.contains
                                        (this.pr.Helpers.get(c)))) {

								this.CandidateHelpers.add(this.pr.Helpers.get(c));

								match = true;
								if (this.Comm_Channels.get(x).equals("BT")) {
							pr.Helpers.get(c).linkWithParent = "BT";
							pr.Helpers.get(c).ctrlMsgDelTimeThroughLinkToParent =
                                          pr.actualBTdelTimeCtrlMsg;
								...
								} else if (this.Comm_Channels.get(x).equals("Zigbee")) {
			pr.Helpers.get(c).linkWithParent = "Zigbee";
				pr.Helpers.get(c).ctrlMsgDelTimeThroughLinkToParent =
                                     pr.actualZigBeeDelTimeCtrlMsg;
									this.task2Runtime = this.msgPayloadReadtime+
                                     pr.actualZigBeeDelTimeHelpObjectMsg;
									this.task3Runtime = this.msgPayloadReadtime+
                                    pr.actualZigBeeDelTimeOppnetTaskMsg;
								} else if (this.Comm_Channels.get(x).equals("WiFi")) {
									pr.Helpers.get(c).linkWithParent = "WiFi";
pr.Helpers.get(c).ctrlMsgDelTimeThroughLinkToParent =
        pr.actualWiFiDelTimeCtrlMsg;
		this.task2Runtime = this.msgPayloadReadtime+ 
	...
	this.task3Runtime = this.msgPayloadReadtime+
				...
				sim_process(this.msgPayloadReadtime);
				sim_trace(1,"Step 2.12a Done: "+ this.get_name()+ ", "+ this.get_id()+
" is done scanning for helpers in range and found the following:");
				...
				if (CandidateHelpers.size() == 0) {
					sim_trace(1, "None");
					sim_trace(1,"Step 2.12b: " + this.get_name() + ", "+ this.get_id()
				...
				int i = 0;
                                Object v2;
                                /* if(this.device=="Sensor"){
                 
                  v2= Seed.graph.insertVertex(Seed.parent, null, "Sensor1", this.location.X, this.location.Y, 80, 30,"shape=image;image=/s.png"); 
                                                 
                }
                //String e="v"+ii;
               else if("Fan" == this.device){
                 v2=Seed.graph.insertVertex(Seed.parent, null, "Fan", this.location.X, this.location.Y, 80, 30, "shape=image;image=/f.png");
                                                } 
                
               else if(this.device=="Billboard"){
                 v2=Seed.graph.insertVertex(Seed.parent, null, "Billboard", this.location.X, this.location.Y, 80, 30, "shape=image;image=/B.jpg");
                                               } 
                
               else if(this.device=="BillBoard2"){
                v2=Seed.graph.insertVertex(Seed.parent, null, "Billboard2", this.location.X, this.location.Y, 80, 30, "shape=image;image=/B.jpg");
                                         } 
                
               else if(this.device=="Smartphone"){
                 v2=Seed.graph.insertVertex(Seed.parent, null, "Smartphone", this.location.X, this.location.Y, 80, 30, "shape=image;image=/s.png");
                                               } 
               else{
               v2=Seed.graph.insertVertex(Seed.parent, null, "Car", this.location.X, this.location.Y, 80, 30, "shape=image;image=/s.png");
                                             } */
                              //  graph.nodeNames.add(Sim_system.get_entity(ev.get_src()).get_name());
                //graph.nodeLoc.add(Inviting_Node.location);
               // System.out.println("--------------------------------------------------------------------------------"+Sim_system.get_entity(ev.get_src()).get_name());
                                 
				while (i < CandidateHelpers.size()) {
					// Step 2.14: Node discovers services in candidate
					// helpers(Node_discover)

	if ((CandidateHelpers.get(i).Comm_Channels.contains("BT") &&
     CandidateHelpers.get(i).OPP)|| (CandidateHelpers.get(i).
Comm_Channels.contains("Zigbee") && CandidateHelpers.get(i).Coordinator)|| 
 CandidateHelpers.get(i).Comm_Channels.contains("WiFi")) {
					...
					if (ev.get_data() != null) {
						...
						if (ev.get_data().equals("Join")
							...
							pr.oppnet.add(Candidate_Helper);
                                                          ...
                                                          he.clear();
                he.add(Candidate_Helper.device);
                he.add(String.valueOf(Candidate_Helper.get_id()));
                he.add(String.valueOf(this.get_id()));
                he.add(Candidate_Helper.linkWithParent);
                
             //  graph.nodeNames.add(this.device);
                graph.nodeLoc.add(Candidate_Helper.location);
                                                        /* if(Candidate_Helper.device=="Sensor"){
                 
                   Object v3= Seed.graph.insertVertex(Seed.parent, null, "Sensor1", Candidate_Helper.location.X, Candidate_Helper.location.Y, 80, 30, "shape=image;image=/s.png"); 
                                                    Seed.graph.insertEdge(Seed.parent, null,"", v2, v3);
                   
                }
                //String e="v"+ii;
               else if(Candidate_Helper.device=="Fan"){
                Object v3=Seed.graph.insertVertex(Seed.parent, null, "Fan", Candidate_Helper.location.X, Candidate_Helper.location.Y, 80, 30, "shape=image;image=/f.png");
                                                Seed.graph.insertEdge(Seed.parent, null,"", v2, v3);
               System.out.println(",,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,");} 
                
               else if(Candidate_Helper.device=="Billboard"){
                Object v3=Seed.graph.insertVertex(Seed.parent, null, "Billboard", Candidate_Helper.location.X, Candidate_Helper.location.Y, 80, 30, "shape=image;image=/B.jpg");
                                                Seed.graph.insertEdge(Seed.parent, null,"", v2, v3);} 
                
               else if(Candidate_Helper.device=="Billboard2"){
                Object v3=Seed.graph.insertVertex(Seed.parent, null, "Billboard2", Candidate_Helper.location.X, Candidate_Helper.location.Y, 80, 30, "shape=image;image=/B.jpg");
                                                Seed.graph.insertEdge(Seed.parent, null,"", v2, v3);} 
                
               else if(Candidate_Helper.device=="Smartphone"){
                Object v3=Seed.graph.insertVertex(Seed.parent, null, "Smartphone", Candidate_Helper.location.X, Candidate_Helper.location.Y, 80, 30, "shape=image;image=/s.png");
                                                Seed.graph.insertEdge(Seed.parent, null,"", v2, v3);} 
               else if(Candidate_Helper.device=="Car"){
                Object v3=Seed.graph.insertVertex(Seed.parent, null, "Car", Candidate_Helper.location.X, Candidate_Helper.location.Y, 80, 30, "shape=image;image=/s.png");
                                               Seed.graph.insertEdge(Seed.parent, null,"", v2, v3);}*/
							...
							this.hlprIntegTime.add((endTime - startTime));
                                                        integ.add((endTime - startTime));
                                                        he.add(String.valueOf(Math.floor((endTime - startTime) * 100) / 100));
                                                        graph.node.add(he);
						...
						} /* end if(ev.get_data().equals("Join") && Candidate_Helper.get_name()
               .equals(Sim_system.get_entity(ev.get_src()).get_name()))*/
							...
							if (ev.get_data().equals("Done..ReqRelease")) {
								...
								sim_trace(
										...
										"Step 2.25 Done: "
												...
												+ ","
												+ this.get_id()
												+ " evaluated the message and source and it is NoGrandKids notification"
                                  + " from "+ Sim_system.get_entity(
														ev.get_src())
														.get_name()
												+ ", "
												+ Sim_system.get_entity(
	...
	public boolean theReleaseMethod(String step) {
		...
		if (ev1.get_data() != null) {
			if (ev1.get_data().equals("Released")) {
				...
				r = true;	}	}
		return r;}
/* This method sorts devices based on their type. This simulates the 
 inviting node ability to
*recognize the type of the device after scanning is done and sends requests
 to devices accordingly.* For example, when a tablet scans BT spectrum 
 for BT devices and it finds a phone, a printer * and a TV it chooses 
        the phone "first" since it is most likely to do the goal task
 */
