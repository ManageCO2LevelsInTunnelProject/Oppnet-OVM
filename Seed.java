...
import static pack.graph.nodeLoc;
import static pack.graph.vertex;
...
//static mxGraph graph = new mxGraph();
  // static  Object parent = graph.getDefaultParent();
	...
	public void body() {
		...
		int AH = 0;
		task2Runtime = pr.task2RuntimeSeedZigBee;
		task3Runtime = pr.task3RuntimeSeedZigBee;
                integ.clear();
                integ.add((double)this.get_id());
		Sim_event ev = new Sim_event();
if(pr.CO2Level>0.6 || pr.CO2Level<0.1){
		// Step 1.1: seed initiates oppnet (Node_initiate)
		Help helpObject = new Help();

			this.oppnetTasks.add("Goal Urgent Circulate fresh air");
		...
		pr.oppnet.add(this);
                ...
                sen.add(this.device);
                sen.add(String.valueOf(this.get_id()));
                graph.node.add(sen);
                graph.nodeLoc.add(this.location);
               
                /*graph.nodeNames.clear();
                graph.nodeLoc.clear();
                graph.invitNode.clear();
        ...
        /*graph.getModel().beginUpdate();
            ...
            Object v1 = graph.insertVertex(parent, null, this.device, this.location.X, this.location.Y, 80, 30, styleName);*/
		...
		// Step 1.2: sensor scans for ad hoc helpers within range(Node_scan)
		...
		for (int i = 0; i < pr.Helpers.size(); i++) {
			...
			if ((pr.Helpers.get(i).Comm_Channels.contains("Zigbee"))
 ...
 graph.invitNode.clear();
		while (i < CandidateHelpers.size()) {
			...
			if ((CandidateHelpers.get(i).Comm_Channels.contains("Zigbee") && CandidateHelpers
					.get(i).Coordinator)) {
				...
				if (this.pr.oppnet.contains(Candidate_Helper)) {
					...
					sim_schedule(Candidate_Helper.get_id(),
							pr.actualZigBeeDelTimeCtrlMsg, 15, "ReqHelp");
					...
					sim_trace(1,
							"Step 1.3.3 Done: " + this.get_name()
									...
									+ Candidate_Helper.get_name() + ", "
									+ Candidate_Helper.get_id());
					Candidate_Helper.linkWithParent = "Zigbee";
					Candidate_Helper.ctrlMsgDelTimeThroughLinkToParent = pr.actualZigBeeDelTimeCtrlMsg;
				}
       ...
       // frame.setContentPane(new JLabel(new ImageIcon(ImageIO.read(new File("tu.gif")))));
        

        frame.setSize((1220*5), (300));
        frame.setVisible(true);
             ...
             //  graph.nodeNames.add(this.device);
                graph.nodeLoc.add(Candidate_Helper.location);
                                     //  if(Candidate_Helper.device!="Fan")
                                      //  graph.invitNode.add(Candidate_Helper.device);
                                       
                                           /*if(Candidate_Helper.device=="Sensor"){
                 
                   Object v2= graph.insertVertex(parent, null, "Sensor1", Candidate_Helper.location.X, Candidate_Helper.location.Y, 80, 30, styleName); 
                                                    graph.insertEdge(parent, null,"", v1, v2);
                   
                }
                //String e="v"+ii;
               else if(Candidate_Helper.device=="Fan"){
                Object v2=graph.insertVertex(parent, null, "Fan", Candidate_Helper.location.X, Candidate_Helper.location.Y, 80, 30, "shape=image;image=/f.png");
                                                graph.insertEdge(parent, null,"", v1, graph.insertVertex(parent, null, "Fan", Candidate_Helper.location.X, Candidate_Helper.location.Y, 80, 30, "shape=image;image=/f.png"));
                System.out.println(",,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,");} 
                
               else if(Candidate_Helper.device=="Billboard"){
                Object v2=graph.insertVertex(parent, null, "Billboard", Candidate_Helper.location.X, Candidate_Helper.location.Y, 80, 30, "shape=image;image=/B.jpg");
                                                graph.insertEdge(parent, null,"", v1, v2);} 
                
               else if(Candidate_Helper.device=="Billboard2"){
                Object v2=graph.insertVertex(parent, null, "Billboard2", Candidate_Helper.location.X, Candidate_Helper.location.Y, 80, 30, "shape=image;image=/B.jpg");
                                                graph.insertEdge(parent, null,"", v1, v2);} 
                
               else if(Candidate_Helper.device=="Smartphone"){
                Object v2=graph.insertVertex(parent, null, "Smartphone", Candidate_Helper.location.X, Candidate_Helper.location.Y, 80, 30, "shape=image;image=/s.png");
                                                graph.insertEdge(parent, null,"", v1, v2);} 
               else if(Candidate_Helper.device=="Car"){
                Object v2=graph.insertVertex(parent, null, "Car", Candidate_Helper.location.X, Candidate_Helper.location.Y, 80, 30, "shape=image;image=/s.png");
                                                graph.insertEdge(parent, null,"", v1, v2);}  */
              
                                 
                                        //  graph.nodeNames.add(Candidate_Helper.device);
                                                    //    graph.nodeLoc.add(Candidate_Helper.location);
                                                      //  graph.invitNode.add(this.device);
					...
					// (Node_report)
					sim_schedule(Candidate_Helper.get_id(),
							pr.task2RuntimeSeedZigBee, 19, helpObject);
					...
					sim_schedule(Candidate_Helper.get_id(),
							pr.task3RuntimeSeedZigBee, 110, oppnetTasks);
       ...
       // frame.setContentPane(new JLabel(new ImageIcon(ImageIO.read(new File("tu.gif")))));
		...
		while (X < AdmittedHelpers.size()) {
			sim_schedule(AdmittedHelpers.get(X).get_id(),
					pr.actualZigBeeDelTimeCtrlMsg, 113, "Released");
			sim_trace(1,
   ...
   pr.oppnet.clear();
   sim_trace(1, "The Co2 level is between 0.1% and 0.6% .."+ this.get_name() + ", " + this.get_id()+"terminated oppnet");
